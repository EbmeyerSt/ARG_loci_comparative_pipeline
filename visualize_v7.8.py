...
def read_db(context_file):
	...
	for id in arg_ids:
		...
		for result in results:
			...
			if not 'name' in gene_dict[id]:
				...
				gene_dict[id]['card_id']=result[7]
				gene_dict[id]['organism']=result[9]
				gene_dict[id]['assembly']=result[10]
				if args.compressed==True:
					gene_dict[id]['cluster_size']=clust_sizes[id]
				gene_dict[id]['env_genes']={}

			gene_dict[id]['env_genes'][result[12]]={}
			gene_dict[id]['env_genes'][result[12]]['env_name']=result[11]
			gene_dict[id]['env_genes'][result[12]]['env_start']=result[-3]
			gene_dict[id]['env_genes'][result[12]]['env_stop']=result[-2]
			gene_dict[id]['env_genes'][result[12]]['env_strand']=result[-1]
	...
	for key, value in gene_dict.iteritems():
		...
		for key2, value2 in value['env_genes'].iteritems():
...
def cluster_seqs(context_file):
	...
	#Use usearch to cluster the sequences at 90%
...
def visualize_phylogeny(gene_dict, context_file):
	...
	tnps=['iscr', 'transpos', 'tnp', 'insertion', '-like', ]
...
def main():
	...
	profiles=[(key,[value2['env_name'] for key2, value2 in value['env_genes'].iteritems()]) for key, value\
	in gene_dict.iteritems()]
